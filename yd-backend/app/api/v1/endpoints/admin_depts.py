...
def list_depts(db: DbDep, _admin: CurrentAdmin):
    """后台：部门列表（树形）。"""
...
def list_depts_flat(db: DbDep, _admin: CurrentAdmin):
...
def create_dept(payload: DeptCreate, db: DbDep, admin: CurrentAdmin):
...
def update_dept(dept_id: int, payload: DeptCreate, db: DbDep, admin: CurrentAdmin):
...
def delete_dept(dept_id: int, db: DbDep, _admin: CurrentAdmin):
